...
public class SessionController {
	...
	public SessionController(SessionRepository sessionRepository,
			ResponseForSessionRepository responseForSessionRepository, ContextUsername contextUsername,
	...
	String session(@PathVariable UUID sessionId, Model model, UriComponentsBuilder uriComponentsBuilder) {
		...
		Optional<ResponseForSession> response = responseForSessionRepository
			.findBySession_SessionIdAndUsername(sessionId, username);
		couponRepository.findByUsername(username).ifPresent(coupon -> {
			model.addAttribute("coupon", coupon);
			model.addAttribute("qrCode", qrCode.dataUrl(
					uriComponentsBuilder.pathSegment("coupons", coupon.getCouponId().toString()).build().toString()));
		});
