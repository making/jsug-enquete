...
public interface Summary<T extends Comparable<T>> {
	...
	default Session asSession() {
		return new Session(getSessionId(), getSessionName(), getSeminarId(), getSeminarName());
	}
	...
	final class Session {
		...
		public String toString() {
			return "Summary.Session(sessionId=" + this.getSessionId() + ", sessionName=" + this.getSessionName()
					+ ", seminarId=" + this.getSeminarId() + ", seminarName=" + this.getSeminarName() + ")";
		}
	...
	class Report<T extends Comparable<T>> {
		...
		@Override
		public String toString() {
			return "Report{" + "average=" + average + ", total=" + total + ", count=" + count + ", details=" + details
					+ '}';
		}
	...
	class SatisfactionReport extends Report<Satisfaction> {
		...
		private static long nsat(Summary.Report<Satisfaction> report) {
			double vsat = extract(report, Satisfaction.EXCELLENT) * 100.0d / report.getCount();
			double dsat = (extract(report, Satisfaction.BAD) + extract(report, Satisfaction.TERRIBLE)) * 100.0d
					/ report.getCount();
			return Math.round(vsat - dsat) + 100;
		}

		private static long extract(Summary.Report<Satisfaction> report, Satisfaction satisfaction) {
			return report.getDetails()
				.stream()
				.filter(d -> d.getValue() == satisfaction)
				.mapToLong(Summary.Detail::getCount)
				.sum();
		}
	...
	class Detail<T extends Comparable<T>> {
		...
		public String toString() {
			return "Summary.Detail(value=" + this.getValue() + ", total=" + this.getTotal() + ", count="
					+ this.getCount() + ")";
		}
