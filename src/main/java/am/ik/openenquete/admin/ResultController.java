...
public class ResultController {
	...
	public ResultController(SeminarRepository seminarRepository, SessionRepository sessionRepository,
			ResponseForSeminarRepository responseForSeminarRepository,
			ResponseForSessionRepository responseForSessionRepository, MessageSource messageSource,
			ObjectMapper objectMapper, SeminarReportService seminarReportService) {
	...
	String seminarForAdmin(@PathVariable UUID seminarId, Model model, Locale locale) throws IOException {
		List<ResponseForSeminar> responses = responseForSeminarRepository.findBySeminar_SeminarId(seminarId);
		Seminar seminar = seminarRepository.findBySeminarId(seminarId).get();
		Map<String, Long> satisfactions = satisfactionMap(
				responses.stream().collect(groupingBy(ResponseForSeminar::getSatisfaction, counting())), locale);
		...
		Map<LocalDateTime, Long> countsForSessions = countsForSession(responseForSessions);
		Map<LocalDateTime, Long> countsForSeminar = countsForSeminar(responses);

		model.addAttribute("seminar", seminar);
		model.addAttribute("satisfactions", satisfactions);
		model.addAttribute("satisfactionsArray", arrayJsonString(satisfactions, "Satisfaction"));
		model.addAttribute("comments", comments);
		model.addAttribute("requests", requests);

		model.addAttribute("countsForSeminar", arrayJsonString(countsForSeminar, "Time"));
		model.addAttribute("countsForSessions", arrayJsonString(countsForSessions, "Time"));
		return "admin/seminar";
	}

	@GetMapping({ "admin/sessions/{sessionId}/result", "speaker/sessions/{sessionId}" })
	String sessionForAdmin(@PathVariable UUID sessionId, Model model, Locale locale) throws IOException {
		List<ResponseForSession> responses = responseForSessionRepository.findBySession_SessionId(sessionId);
		...
		double nsatAverage = satisfactionReportMap.values()
			.stream()
			.mapToLong(Summary.SatisfactionReport::getNsat)
			.summaryStatistics()
			.getAverage();
		double satisfactionAverage = satisfactionReportMap.values()
			.stream()
		...
		long nsat = report.map(Summary.SatisfactionReport::getNsat).orElse(0L);
		double satisfaction = report.map(Summary.SatisfactionReport::getAverage).orElse(0.0);
		Map<String, Long> satisfactions = satisfactionMap(
				responses.stream().collect(groupingBy(ResponseForSession::getSatisfaction, counting())), locale);
		Map<String, Long> difficulties = difficultyMap(
				responses.stream().collect(groupingBy(ResponseForSession::getDifficulty, counting())), locale);
		List<String> comments = responses.stream() //
		...
		model.addAttribute("difficultiesArray", arrayJsonString(difficulties, "Difficulty"));
		model.addAttribute("comments", comments);
	...
	String arrayJsonString(Map<?, Long> map, Supplier<List<Object[]>> listSupplier) throws IOException {
		return objectMapper.writeValueAsString(map.entrySet()
			.stream()
	...
	Map<String, Long> satisfactionMap(Map<Satisfaction, Long> map, Locale locale) {
		...
		return satisfactions.entrySet()
			.stream()
			.collect(
					toMap(e -> satisfaction(e.getKey(), locale), Map.Entry::getValue, (k, v) -> v, LinkedHashMap::new));
	}
	...
	Map<String, Long> difficultyMap(Map<Difficulty, Long> map, Locale locale) {
		...
		return difficulties.entrySet()
			.stream()
			.collect(toMap(e -> difficulty(e.getKey(), locale), Map.Entry::getValue, (k, v) -> v, LinkedHashMap::new));
	}

	String satisfaction(Satisfaction satisfaction, Locale locale) {
		return messageSource.getMessage("satisfaction." + satisfaction.name().toLowerCase(), null, locale);
	}

	String difficulty(Difficulty difficulty, Locale locale) {
		return messageSource.getMessage("difficulty." + difficulty.name().toLowerCase(), null, locale);
	}

	Map<LocalDateTime, Long> countsForSession(List<ResponseForSession> responseForSessions) {
	...
	Map<LocalDateTime, Long> countsForSeminar(List<ResponseForSeminar> responseForSeminar) {
